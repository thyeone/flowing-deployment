...
import { Controller, SubmitHandler, type UseFormReturn, useForm, useWatch } from 'react-hook-form';
...
import { sliceLabelToTitle, sliceOriginalResponse } from '../utils';
...
export default function MultiValueResponsePopup({
  ...
  const { data: question } = useGetValueQuestion(type);

  const { setValue, watch } = hookForm;
  const { control, handleSubmit, resetField } = useForm();

  const newResponse = useWatch({
    control,
  });
  ...
  return (
    <PopupContainer key={type} isOpen={isOpen}>
      <Header>
        ...
        <Header.Center>{label + ` 편집`}</Header.Center>
      </Header>
      <h1 className="whitespace-pre-wrap text-xl font-bold">{`${sliceLabelToTitle(
        label,
      )}에 대한\n나의 가치관을 작성해주세요`}</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Spacing size={22} />
        <ButtonWrapper>
          <Button disabled={!Object.values(newResponse).filter((value) => value).length}>
            수정
          </Button>
        </ButtonWrapper>
      </form>
